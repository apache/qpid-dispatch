/*
	Source: https://github.com/jwstadler/angular-jquery-dialog-service
	License: MIT
*/
angular.module('dialogService', []).service('dialogService',
	...
	function($rootScope, $q, $compile, $templateCache, $http) {
			...
			this.isOpen = function(id) {
				// get the dialog, but return false instead of throwing an exception
				try {
					var dialog = getExistingDialog(id);
					// return the isOpen value of the underlying jqueryui dialog
					var open = dialog.ref.dialog("isOpen");
					console.log("dislog-service: isOpen returns " + open);
					return dialog.ref.dialog("isOpen");
				}
		        catch(err) {
		        	console.log("dialog-service: isOpen caught an error " + err);
					return false;
				}
			};
