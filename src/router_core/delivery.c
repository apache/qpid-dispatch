...
static bool qdr_delivery_has_peer_CT(const qdr_delivery_t *dlv);
...
void qdr_delivery_set_tag_sent(qdr_delivery_t *delivery, bool tag_sent)
{
    if (!delivery)
        return;
...
#if 0
static void qdr_delivery_export_transfer_state(qdr_delivery_t *dlv, pn_delivery_t* pdlv)
{
    qdr_delivery_write_extension_state(dlv, pdlv, true);
}


static void qdr_delivery_export_disposition_state(qdr_delivery_t *dlv, pn_delivery_t* pdlv)
{
