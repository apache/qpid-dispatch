                                      ...
                                      qd_field_iterator_t *identity)
{
    ...
    if (!name && !identity) {
        ...
        qd_log(query->log_source, QD_LOG_ERROR, "Error performing DELETE of %s: %s", CONFIG_AUTOLINK_TYPE, query->status.description);
    }
    else {
        if (identity)
                                      ...
                                      qd_parsed_field_t   *in_body)
{
    while (true) {
        ...
        if (!!al) {
            ...
            qd_log(query->log_source, QD_LOG_ERROR, "Error performing CREATE of %s: %s", CONFIG_AUTOLINK_TYPE, query->status.description);
            break;
        }
                                ...
                                const char          *qdr_config_auto_link_columns[])
{
    ...
    if (!name && !identity) {
        query->status = QD_AMQP_BAD_REQUEST;
        query->status.description = "No name or identity provided";
        qd_log(query->log_source, QD_LOG_ERROR, "Error performing READ of %s: %s", CONFIG_AUTOLINK_TYPE, query->status.description);
    }
    else {
