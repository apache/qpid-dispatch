 ...
 */
static void set_terminus_address(qdr_link_t *link)
{
    qd_link_t *qd_link = (qd_link_t *) qdr_link_get_context(link);
    char *terminus_addr = (char*)pn_terminus_get_address(pn_link_remote_target((pn_link_t  *)qd_link_pn(qd_link)));
    if (terminus_addr) {
