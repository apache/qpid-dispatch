...
static void connection_decorate(qd_server_t *server, qd_connection_t *conn, const qd_server_config_t *config)
{
    ...
    if (config) {
        ...
        // Append any user-configured properties. conn_props is a
        // pn_data_t PN_MAP type. Append the map elements - not the
        // map itself!
        if (config->conn_props) {
            ...
            for (size_t i = 0; i < count / 2; ++i) {
                // The key must be of type Symbol.  The Python agent
                // has validated the keys as ASCII strings, but the
                // JSON converter does not provide a Symbol type so
                // all the keys in conn_props are PN_STRING.
                pn_data_next(config->conn_props);

                assert(pn_data_type(config->conn_props) == PN_STRING);

                pn_data_put_symbol(props, pn_data_get_string(config->conn_props));

                // Put the value
                pn_data_next(config->conn_props);
                qdpn_data_insert(props, config->conn_props);
            }

            pn_data_exit(config->conn_props);
        }
    }
