...
static void handle_outgoing(qdr_tcp_connection_t *conn)
{
    if (conn->outstream) {
        ...
        // kgiusti: do something smarter and less often than this:
    ...
    if (link_context) {
        ...
        if (disp == PN_RECEIVED) {
            ...
            if (!dstate) {
                ...
                window_opened = tc->unacked_octets < TCP_MAX_CAPACITY;
                qd_log(tcp_adaptor->log_source, QD_LOG_INFO,
                       "[C%"PRIu64"] TCP backpressure window update: %s "
                       " unacked=%"PRIu64" local rx-seq=%"PRIu64" remote tx-seq=%"PRIu64,
                       tc->conn_id, window_opened ? "OPEN" : "CLOSED", tc->unacked_octets,
                       tc->rx_sequence, dstate->section_offset);
            }
