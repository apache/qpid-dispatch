...
static void handle_connection_event(pn_event_t *e, qd_server_t *qd_server, void *context)
{
    ...
    case PN_RAW_CONNECTION_READ: {
        ...
        conn->bytes_in += read;
        qd_log(log, QD_LOG_DEBUG, "[C%"PRIu64"] PN_RAW_CONNECTION_READ Read %i of %"PRIu64" bytes", conn->conn_id, read, conn->bytes_in);
        while (qdr_connection_process(conn->qdr_conn)) {}
        break;
    }
    case PN_RAW_CONNECTION_WRITTEN: {
        ...
        qd_log(log, QD_LOG_DEBUG, "[C%"PRIu64"] PN_RAW_CONNECTION_WRITTEN Wrote %zu of %"PRIu64" bytes", conn->conn_id, written, conn->bytes_out);
        while (qdr_connection_process(conn->qdr_conn)) {}
        break;
    }
    default:
...
static void qdr_del_tcp_connection_CT(qdr_core_t *core, qdr_action_t *action, bool discard)
{
    if (!discard) {
        ...
        if (conn->in_list) {
            ...
            qd_log(tcp_adaptor->log_source, QD_LOG_DEBUG,
                   "[C%"PRIu64"] qdr_del_tcp_connection_CT %s (%zu) bytes_in=%"PRIu64", bytes_out=%"PRId64
