    ...
    if (conn->outgoing_body_data == 0) {
        ...
        if (body_data_result == QD_MESSAGE_BODY_DATA_OK) {
            ...
            case QD_MESSAGE_BODY_DATA_NO_MORE:
                qd_log(tcp_adaptor->log_source, QD_LOG_INFO, "[C%i] EOS", conn->conn_id);
                break;
            case QD_MESSAGE_BODY_DATA_INVALID:
                qd_log(tcp_adaptor->log_source, QD_LOG_ERROR, "[C%i] Invalid body data for streaming message", conn->conn_id);
                break;
        ...
        for (size_t i = 0; i < used; i++) {
            if (conn->outgoing_buffs[conn->outgoing_buff_idx + i].bytes) {
                ...
                qd_log(tcp_adaptor->log_source, QD_LOG_ERROR,
                       "[C%i] empty buffer can't be written (%i of %i)", conn->conn_id, i+1, used);
            }
        }
        qd_log(tcp_adaptor->log_source, QD_LOG_DEBUG,
               "[C%i] Writing %i bytes", conn->conn_id, bytes_written);
    }
    return result;
}
