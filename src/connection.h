...
struct qd_connection_t {
    ...
    uint64_t connection_id;  // A unique identifier for the qd_connection_t. The underlying pn_connection already has
                             // one but it is long and clunky
    const char* user_id;     // A unique identifier for the user on the connection. This is currently populated from the
                             // client ssl cert. See ssl_uid_format in server.h for more info
    bool                    free_user_id;
