...
qd_router_link_t* qd_router_link(qd_link_t *link, qd_link_type_t link_type, qd_direction_t direction, qd_address_t *owning_addr) {
	...
	const qd_server_config_t *config = connection->listener->config;
...
static int router_incoming_link_handler(void* context, qd_link_t *link)
{
    ...
    qd_router_link_t *rlink = qd_router_link(link, is_router ? QD_LINK_ROUTER : QD_LINK_ENDPOINT, QD_INCOMING, 0);
...
static int router_outgoing_link_handler(void* context, qd_link_t *link)
{
    ...
    //
    qd_router_link_t *rlink = qd_router_link(link, is_router ? QD_LINK_ROUTER : QD_LINK_ENDPOINT, QD_OUTGOING, 0);
