...
qd_iterator_t *qd_parse_raw(qd_parsed_field_t *field)
{
    if (!field)
        return 0;
    if (!field->raw_iter) {
        field->raw_iter = qd_buffer_field_iterator(&field->amqp.value,
                                                   ITER_VIEW_ALL);
    }
...
qd_iterator_t *qd_parse_typed(qd_parsed_field_t *field)
{
    if (!field)
        return 0;
    if (!field->typed_iter) {
        field->typed_iter = qd_buffer_field_iterator(&field->full_field,
                                                     ITER_VIEW_ALL);
    }
    ...
    uint32_t              *blob_item_count)
{
    ...
    *blob_item_count        = 0;
    ...
    qd_iterator_pointer_t iptr;
    qd_iterator_get_view_cursor(ma_iter_in, &iptr);
