...
static void listener_start(qd_http_listener_t *hl, qd_http_server_t *hs)
{
    ...
    m->origin = (config->http_root_dir && *config->http_root_dir) ? /* File system root */
                    config->http_root_dir
                                                                  : QPID_CONSOLE_STAND_ALONE_INSTALL_DIR;
                            ...
                            void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_HTTP_WRITEABLE:
    {
        ...
        {
            if (lws_add_http_header_status(wsi, HTTP_STATUS_OK, &position, end) || add_header_by_name(wsi, "content-type:", "text/plain", &position, end) || add_header_by_name(wsi, "connection:", "close", &position, end))
                return 1;
                            ...
                            void *user, void *in, size_t len)
{
    ...
    case LWS_CALLBACK_HTTP_WRITEABLE:
    {
        ...
        if (!stats->headers_sent)
        {
            if (lws_add_http_header_status(wsi, HTTP_STATUS_OK, &position, end) || add_header_by_name(wsi, "content-type:", "text/plain", &position, end) || lws_add_http_header_content_length(wsi, 3, &position, end))
                return 1;
            if (lws_finalize_http_header(wsi, &position, end))
                return 1;
            stats->headers_sent = true;
...
void qd_http_server_free(qd_http_server_t *hs)
{
    ...
    if (hs->context)
        lws_context_destroy(hs->context);
    free(hs);
}
...
static qd_http_listener_t *wsi_listener(struct lws *wsi)
{
    ...
    if (vhost)
    { /* Get qd_http_listener from vhost data */
