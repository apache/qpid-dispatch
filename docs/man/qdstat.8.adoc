...
Neighbor Legacy Mode::
If this is 'on', this router has at least one connected neighbor that is running an older version of the routing protocol.

Network Legacy Mode::
If this is 'on', there is at least one interior router in the network that is running an older version of the routing protocol.  This means that the interior routers are running in legacy-compatibility mode which adds some overhead to the operation of the network.
...
cred::
For outgoing links: The number of flow credits currently available on this link; for incoming links: The established credit loop based on end-to-end flow control.
...
inLocal::
The number of incoming links for this address attached to the local router.

inRemote::
The number of incoming links for this address on all remote interior routers.

credLocal::
The aggregate amount of credit issued for this address by locally attached receivers.  Each outgoing link's credit it limited by the configured capacity.

credRemote::
The total amount of credit issued for this address by receivers attached to remote interior routers.
