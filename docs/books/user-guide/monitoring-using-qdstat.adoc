...
A state dump shows the current operational state of the router network. The following information is displayed for each router:
...
The type of router on which you run the `qdstat` command determines which routers are included in the state dump. If you create the state dump on an interior router, then it includes all interior routers. If you create the state dump on an edge router, then it includes only that edge router.
