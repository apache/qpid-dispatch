...
The resource limits defined in a connector vhost policy are applied to links initiated by the external AMQP container. The connector vhost policy does not restrict links created by the router.

A connector vhost policy can only be applied to a connector with a `normal` or `route-container` role. You can not apply connector vhost policies to connectors with `inter-router` or `edge` roles.
...
+
--
This example applies the connector vhost policy configured in the previous step:
