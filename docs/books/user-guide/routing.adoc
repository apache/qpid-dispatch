...
+
For a multicast address, the router associates the incoming delivery with all outbound deliveries. When each outbound delivery is settled by its consumer, the router sets a final delivery state for the original inbound delivery and passes it to the producer.
+
...
* At least one consumer received the message, but no consumers rejected it.
