...
A fallback destination is a queue on a broker that stores messages that are not directly routable to any consumers.
...
[NOTE]
====
{RouterName} does not preserve delivery order for messages stored at a fallback destination. When a consumer reconnects, it will receive the stored messages interleaved with any new messages sent by producers.
====
...
However, the original delivery order of the queued messages is not preserved; the queued messages will be interleaved with the new messages.
