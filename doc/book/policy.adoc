...
| enableVhostPolicy   | true       | Enable vhost policy connection denial, and resource limit enforcement.
...
|====
| attribute                   | default  | description
| name                        |          | Vhost name. A unique name is required.
| maxConnections              | 65535    | Maximum number of concurrent client connections allowed
| maxConnectionsPerUser       | 65535    | Maximum number of concurrent client connections allowed for any single user.
...
| maxFrameSize         | 2^32-1  | Largest frame that may be sent on this connection. (AMQP Open, max-frame-size)
...
| maxSessionWindow     | 2^32-1  |Incoming capacity for new sessions. (AMQP Begin, incoming-window)
| *AMQP Link Attach*             | |
...
| maxSenders           | 2^32-1  | Maximum number of sending links that may be created on this connection.
| maxReceivers         | 2^32-1  | Maximum number of receiving links that may be created on this connection.
