...
`maxConnections`::
The maximum number of concurrent client connections allowed for this router. This limit is always enforced, even if no other policy settings have been defined. The limit is applied to all incoming connections regardless of remote host, authenticated user, or targeted vhost. The default value is `65535`.
...
`policyDir`:: 
The absolute path to a directory that holds vhost policy definition files in JSON format (`*.json`). The router processes all of the vhost policies in each JSON file that is in this directory. For more information, see xref:configuring_vhost_policies_json[].
...
[[configuring_vhost_policies_router]]
...
`id`::
The host name of the vhost. This vhost policy will be applied to any client connection that is directed to the hostname that you specify.
...
====
In this example, users in this user group can only connect to the vhost from localhost:
...
+
You can use the substitution token `{user}` to specify an address that contains a user's authenticated user name. This enables you to allow access to resources specific to each user in the user group without having to name each user individually. You can only specify the `{user}` token once in an address name. If there are multiple tokens in an address, only the leftmost token will be substituted.
+
You can use an asterisk (`*`) wildcard to match one or more characters in an address. However, this wildcard is only recognized if it is the last character in the address name.
+
...
[options="nowrap"]
----
sources: tmp_{user}, temp*, {user}-home-* // <1>
----
<1> This definition allows access to any address that meets any of the following rules:
...
`targets`::
A list of target addresses from which users in this group may send messages. You can specify multiple addresses and use user name substitution and wildcards the same way as with source addresses.
--
...
[[configuring_vhost_policies_json]]
...
+
The vhost policy is configured the same way as a `vhost` entity in the router configuration file, only using JSON syntax. For more information about vhost policy attributes, see xref:configuring_vhost_policies_router[].
+
